...
def _normalize_slot(value: Any, fallback_index: int | None = None) -> str | None:
    ...
    match = re.fullmatch(
        r"(?:slot|element|subject|object|text|graphic)[_-]?([a-e])",
        slot,
    )
    if match:
...
def sanitize_layout(
    raw: Any,
    *,
    width_override: Any = None,
    height_override: Any = None,
    ...
    base_width = source.get("width", canvas.get("width", 1024))
    base_height = source.get("height", canvas.get("height", 1024))
    width = _coerce_int(
        width_override if width_override is not None else base_width,
        1024,
        MIN_CANVAS_SIZE,
        MAX_CANVAS_SIZE,
    )
    height = _coerce_int(
        height_override if height_override is not None else base_height,
        1024,
        MIN_CANVAS_SIZE,
        MAX_CANVAS_SIZE,
    )
    ...
    if grid not in {"none", "thirds", "cross", "quarters"}:
        grid = "thirds"
    ...
    for index, item in enumerate(_extract_boxes(source)):
        ...
        if not active or bbox is None:
            continue
        if slot in by_slot:
            warnings.append(f"Duplicate box for slot {slot.upper()} was replaced by the last value.")
        by_slot[slot] = {
            "slot": slot,
            "ui_color": UI_COLORS[slot],
            "bbox_2d": bbox,
        }
    ...
    layout = {
        ...
        "canvas": {
            "width": width,
            "height": height,
            "aspect_ratio": simplified_aspect_ratio(width, height),
            "coordinate_space": "normalized_0_1000",
            "bbox_format": "xyxy",
            "grid": grid,
...
def sanitize_config(raw: Any) -> tuple[dict[str, Any], list[str]]:
    ...
    if result["ui_language"] not in {"auto", "en", "ja"}:
        result["ui_language"] = "auto"
    result["scene_description"] = _clean_text(source.get("scene_description"), MAX_SCENE_LENGTH)
    result["duration_seconds"] = _coerce_float(
        source.get("duration_seconds"), 5.0, 0.0, 300.0
    )
    result["compiler_mode"] = _choice(
        source.get("compiler_mode"), _ALLOWED_COMPILER_MODES, "hybrid"
    )
    result["output_format"] = _choice(
        source.get("output_format"), _ALLOWED_OUTPUT_FORMATS, "direct"
    )
    result["schema_profile"] = _choice(
        source.get("schema_profile"), _ALLOWED_SCHEMA_PROFILES, "verified_split_bbox"
    )
    result["reinforcement"] = _choice(
        source.get("reinforcement"), _ALLOWED_REINFORCEMENT, "balanced"
    )
    ...
    result["custom_instruction"] = _clean_text(
        source.get("custom_instruction"), MAX_CUSTOM_INSTRUCTION_LENGTH
    )

    source_slots = source.get("slots")
    if isinstance(source_slots, list):
        source_slots = {
            str(item.get("slot", "")).lower(): item
    ...
    for slot in SLOTS:
        ...
        clean["type"] = _choice(item.get("type"), _ALLOWED_TYPES, clean["type"])
        clean["description"] = _clean_text(
            item.get("description"), MAX_DESCRIPTION_LENGTH
        )
        clean["exact_text"] = _clean_text(
            item.get("exact_text"), MAX_EXACT_TEXT_LENGTH
        )
        clean["motion"] = _choice(item.get("motion"), _ALLOWED_MOTIONS, "static")
        clean["value"] = _coerce_float(item.get("value"), None, -100000.0, 100000.0)
        clean["phase"] = _coerce_int(item.get("phase"), SLOTS.index(slot) + 1, 1, 99)
        clean["custom_behavior"] = _clean_text(
            item.get("custom_behavior"), MAX_DESCRIPTION_LENGTH
        )
    ...
    if motion not in _ALLOWED_CAMERA_MOTIONS:
        motion = "Static Shot"
    speed = str(camera_source.get("speed", "auto")).strip().lower()
    amplitude = str(camera_source.get("amplitude", "auto")).strip().lower()
    result["camera"] = {
        "motion": motion,

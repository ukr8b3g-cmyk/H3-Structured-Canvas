...
function normalizeTrack(track) {
  ...
  track.keys = Array.isArray(track.keys) ? track.keys : [];
  return track;
}

function trackEmpty(track) {
  const safe = normalizeTrack(track);
  return !safe?.start && !safe?.end && !safe.keys.length;
}
...
function pointAt(track, t) {
  ...
  for (let index = 0; index < safe.keys.length; index += 1) {
    if (Math.abs(Number(safe.keys[index].time) - t) <= EPS) {
      return { kind: "key", index, time: Number(safe.keys[index].time) };
    }
  }
  return null;
}
...
function forceMultiKeyEditState(controller) {
  const exp = controller?.__h3scTimelineExp;
  if (!exp) return;
  const slot = currentSlot(controller);
  const track = currentTrack(controller, slot);
...
function installCanvasGateFix(controller) {
  ...
  const begin = (event) => {
    ...
    if (hit && editable) {
      controller.__h3scMultiKeyGateFixDrag = {
